 ...
 */
export const commonParameters: Record<string, ParameterDefinition> = {
  seed: {
    name: 'seed',
    label: 'Random Seed',
    type: 'string',
 ...
 */
export const createLegacyAdapter = (): LegacyAlgorithmAdapter => {
  return {
    /**
    ...
    wrapFunction(fn: Function, metadata: Partial<Algorithm>): Algorithm {
      return {
        ...
        generate(params: ParameterSet, config: AlgorithmConfig): AlgorithmOutput {
          try {
 ...
 */
function convertGridToPaths(grid: any[][]): number[][][] {
  const paths: number[][][] = [];
 ...
 */
export function createParametersFromObject(params: Record<string, any>): ParameterDefinition[] {
