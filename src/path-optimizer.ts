...
/**
 * Path Optimizer for Pen Plotter
 * Provides utilities for optimizing SVG paths for efficient plotting
 * Based on drawingbots.net research and TSP algorithms
 */
...
export class PathOptimizer {
   ...
   * Reduces points while maintaining shape within tolerance
   */
  simplifyPath(points: Point[], tolerance: number = 1.0): Point[] {
   ...
   * Reduces pen lifts by connecting nearby paths
   */
  joinPaths(paths: PathSegment[], threshold: number = 5.0): PathSegment[] {
    const joined: PathSegment[] = [];
   ...
   * Minimizes total pen travel distance
   */
  optimizeOrder(paths: PathSegment[]): PathSegment[] {
   ...
   * Optimizes for pen plotter efficiency
   */
  cleanSVG(svgString: string): string {
    ...
    // Normalize number format (remove unnecessary decimals)
   ...
   */
  parseSVGPath(pathData: string): PathSegment[] {
   ...
   */
  toSVGPath(segments: PathSegment[]): string {
